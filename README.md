...
現在は、動画に手動で付けられた日本語字幕のみ対応しています。日本語以外の字幕と、YouTube の自動生成字幕は対象外です。
...
- 対応字幕: 手動で付けられた日本語字幕のみ
- 非対応: 日本語以外の字幕、YouTube の自動生成字幕（文字ごとに出るカラオケ型字幕を含む）
- プレーヤー横のトグルで、読みやすい改行の **オン / オフ** を切り替え
- 整形中の字幕は Immersive Translate と同じ方式: YouTube 標準字幕を隠し、自前オーバーレイだけを表示。字幕をドラッグすると **縦位置だけ** 移動できます（ホストごとに記憶）
